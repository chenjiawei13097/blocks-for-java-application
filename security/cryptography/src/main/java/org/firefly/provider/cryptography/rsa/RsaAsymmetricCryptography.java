...
public class RsaAsymmetricCryptography implements AsymmetricCryptography<byte[], byte[]> {
    ...
    private static final String NO_RSA_ALGORITHM_PROVIDER =
            String.format("No provider which implements the %s algorithm is found", RSA_ALGORITHM);
    private static final String NO_SHA256_WITH_RSA_ALGORITHM_PROVIDER =
    ...
    public KeyPair<byte[], byte[]> generateKeyPair() throws CryptographyException {
        ...
        try {
            ...
            throw new CryptographyException(NO_RSA_ALGORITHM_PROVIDER, e);
        }
    ...
    private Key fromBytes(byte[] bytesKey, boolean isPrivate) throws CryptographyException {
        ...
        try {
            ...
            throw new CryptographyException(
                    String.format("Fails to get key from %s", Hex.encodeHexString(bytesKey, true)), e);
        }
    ...
    public byte[] sign(byte[] message, byte[] privateKey) throws CryptographyException {
        ...
        try {
            ...
            throw new CryptographyException(NO_SHA256_WITH_RSA_ALGORITHM_PROVIDER, e);
        }
        ...
        try {
            ...
            throw new CryptographyException(String.format(
                    "Fails to initialize for signing with key %s", Hex.encodeHexString(privateKey, true)), e);
        }
    ...
    public boolean verify(byte[] message, byte[] messageDigest, byte[] publicKey) throws CryptographyException {
        ...
        try {
            ...
            throw new CryptographyException(String.format(
                    "Fails to initialize for verification with key %s", Hex.encodeHexString(publicKey, true)), e);
        }
